...
- July 30, 2025: Updated job fetching schedule from 8 AM and 7 PM to 8 AM and 1 PM for better midday coverage of new humanitarian job postings
- July 30, 2025: Fixed critical rich text editor paste functionality to properly clean Microsoft Office HTML/CSS formatting - implemented comprehensive content sanitization that removes MSO styling codes, font specifications, line-height formatting, color codes, and tab-stops when users paste from Word documents, ensuring clean text display instead of raw HTML/CSS codes appearing in job descriptions
...
- July 24, 2025: Fixed job details page blank screen issue caused by Vite React plugin error - resolved "@vitejs/plugin-react can't detect preamble" error by adding explicit React import to job-details.tsx, fixed TypeScript null/undefined type errors by converting null values to undefined for proper type compatibility with SEOHead component props, ensured proper React detection by Vite build system
- July 26, 2025: Fixed critical cursor positioning issue in SimpleRichTextEditor - replaced unreliable Range-based cursor saving with text offset-based tracking system, implemented proper content change detection to prevent unnecessary DOM updates, removed conflicting innerHTML updates, added update flags to prevent cursor interference during changes, completely resolved cursor jumping during backspace/delete operations for smooth text editing experience
- July 26, 2025: Enhanced image resize and drag functionality in rich text editor - improved image interaction event handling with better click detection, enhanced resize handle positioning and visibility with larger handles and better styling, fixed container positioning for proper handle placement, added comprehensive console logging for debugging, improved image selection states with blue borders and delete buttons, made images draggable with proper drag-and-drop functionality
- July 26, 2025: Implemented comprehensive table resizing functionality - added automatic table styling for pasted Excel tables with proper borders and cell formatting, created column resize handles with blue hover indicators, implemented fixed table layout for consistent column sizing, added individual cell resize handles with mouse drag functionality, applied column-wide resizing that updates all cells in the same column, enhanced table interactivity with better visual feedback and cursor styles
- July 29, 2025: Implemented comprehensive Google Jobs Schema (JobPosting structured data) for enhanced SEO - added JobStructuredData component with Google Jobs 2025 compliance including required fields (title, description, datePosted, hiringOrganization, jobLocation), smart employment type detection (CONTRACTOR, FULL_TIME, PART_TIME, INTERN), country code mapping (Kenya→KE, Somalia→SO), clean HTML descriptions with 5000 char limits, proper YYYY-MM-DD date formatting, experience mapping to months, and dynamic schema generation across home, jobs, tenders, and individual job detail pages
- July 29, 2025: Fixed critical SEO canonical URL and meta tag issues - corrected server-side rendering to set unique canonical URLs for each job page instead of defaulting to homepage, implemented job-specific meta name="title" and meta name="description" tags that accurately describe individual job listings, updated client-side SEOHead component to properly manage unique meta tags per page, ensuring proper search engine indexing and social media sharing with correct job-specific URLs and metadata
- July 29, 2025: Resolved duplicate Open Graph tags and Twitter URL inconsistencies - eliminated conflicting og:type declarations by ensuring job pages have single og:type="article" instead of duplicate website/article tags, fixed Twitter URL meta tags to point to job-specific URLs rather than homepage, implemented proper server-side meta tag replacement to prevent social media crawler confusion, ensuring clean metadata for optimal search engine and social platform compatibility
- July 29, 2025: Implemented complete server-side rendering (SSR) for job pages to ensure full content accessibility to search engine crawlers - replaced empty div id="root" containers with fully rendered HTML job content including job descriptions, qualifications, application instructions, job details sidebar, and complete page layout, added automatic text formatting conversion (markdown-style **bold** to HTML strong tags, email address auto-linking), ensuring search engines with limited JavaScript rendering capabilities can access complete job content for optimal SEO indexing
- July 29, 2025: Optimized JobPosting structured data for Google Jobs compliance - cleaned and truncated job descriptions to under 800 characters removing HTML tags, markdown formatting, and excessive whitespace per Google recommendations, added comprehensive hreflang tags for international SEO targeting English variants (en, en-US, en-KE, en-SO) and x-default fallback on both homepage and job pages, ensuring optimal search engine indexing and regional targeting for Somalia and Kenya job searches
- July 30, 2025: Expanded job fetching to cover all East African countries - added Ethiopia, Uganda, and Tanzania to ReliefWeb API fetching system alongside existing Kenya and Somalia coverage, implemented enhanced location detection for major cities across all 5 countries (Addis Ababa, Kampala, Dar es Salaam, etc.), adjusted fetch limits to 15 jobs per country for optimal API usage, updated all SEO content and homepage messaging to reflect comprehensive East Africa coverage with 500+ job opportunities across the region
- July 30, 2025: Completely resolved Microsoft Office paste formatting issues in rich text editor - implemented comprehensive cleaning solution using DOM-based text extraction to strip all HTML/CSS artifacts completely, enhanced client-side cleanText function with specific patterns to remove CSS fragments like "level1 lfo2", "color:#002060", "font-family" codes that appear as text remnants, applied database cleanup operations to remove existing formatting artifacts from stored job content, created both server-side rendering cleaning and client-side display cleaning for complete coverage, ensuring clean text display when users paste Word document content into job creation forms
- July 30, 2025: Enhanced super admin permissions to allow editing and updating any job posting regardless of creator - modified PUT /api/jobs/:id route to check both job ownership and admin status, allowing super admins to edit jobs posted by other users or scraped from external sources, also applied same permissions to DELETE operations for complete admin control over job management
